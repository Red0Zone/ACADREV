...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!allowed.includes(ext)) {
      return cb(new Error(`File type ${ext} not allowed. Accepted formats: ${allowed.join(', ')}`));
    }
    cb(null, true);
  }
});
    ...
    upload.single('file')(req, res, (err) => {
      if (err instanceof multer.MulterError) {
        ...
        if (err.code === 'LIMIT_UNEXPECTED_FILE') {
          return res.status(400).json({ message: 'Unexpected file field, allowed formats: pdf, docx, png, jpg, jpeg, txt' });
        }
  ...
  async (req, res) => {
    try {
      ...
      if (!responseId || isNaN(responseId)) {
        return res.status(400).json({ message: 'Invalid or missing response ID' });
      }

      // Check if response exists
      const [responseExists] = await db.promise().query(
        'SELECT id FROM qlt_responses WHERE id = ?',
        [responseId]
      );
      ...
      if(!file && !url) {
        return res.status(400).json({ message: 'No file or URL provided for evidence' });
      }
      ...
      if (url) {
        ...
        try {
          ...
          return res.status(200).json({ message: 'URL evidence added successfully' });
        } catch (dbErr) {
          console.error('Database error (URL):', dbErr);
          return res.status(500).json({ message: 'Failed to save URL evidence to database' });
        }
      }
      ...
      try {
        ...
        res.status(500).json({ message: 'Failed to save file evidence to database' });
      }
    }
    } catch (err) {
...
const getEvidence = async (req, res) => {
  try {
    ...
    if (files.length === 0) {
      return res.status(404).json({ message: 'No evidence found for this response' });
    }

    res.status(200).json(files);
  } catch (err) {
    console.error('Unexpected error:', err);
    res.status(500).json({ message: 'An unexpected error occurred while fetching evidence' });
  }
};
